...
# pylint: disable=C0103,C0111,C0302,C0411,C0413
# pylint: disable=E0611,F0401,R0201,R0204,R0915,W0232
...
# PyPI imports
import pmisc
from pmisc import AE, AI, APROP, AROPROP, GET_EXMSG, RE
import pytest
if sys.hexversion < 0x03000000:
    import mock
...
class TestCsvFile(object):
    ...
    def test_init_exceptions(self):
        ...
        for extype, exmsg, fobj in func_pointers:
            with pytest.raises(extype) as excinfo:
                with pmisc.TmpFile(fobj) as fname:
                    pcsv.CsvFile(fname=os.path.normpath(fname))
    ...
    def test_eq(self):
        ...
        with pmisc.TmpFile() as fname:
            pcsv.write(fname, [['a'], [1]], append=False)
            obj1 = pcsv.CsvFile(fname, dfilter='a')
            obj2 = pcsv.CsvFile(fname, dfilter='a')
        with pmisc.TmpFile() as fname:
            pcsv.write(fname, [['a'], [2]], append=False)
            obj3 = pcsv.CsvFile(fname, dfilter='a')
        assert obj1 == obj2
        assert obj1 != obj3
        assert not obj3 == 5
    ...
    def test_str(self):
        ...
        with pmisc.TmpFile(write_file) as fname:
            obj = pcsv.CsvFile(fname=os.path.normpath(fname))
        ref = (
            'File: {0}\n'
    ...
    def test_data_start(self):
        ...
        with pmisc.TmpFile(write_data_start_file) as fname:
            obj = pcsv.CsvFile(fname=fname)
        assert obj.data() == [[2, None, 30], [2, 5, 40], [3, 5, 50]]
        with pmisc.TmpFile(write_no_header_file) as fname:
            obj = pcsv.CsvFile(fname=fname, has_header=False)
        assert obj.data() == [[1, 4, 7], [2, 5, 8], [3, 6, 9], [1, 6, 6]]
        with pmisc.TmpFile(write_data_start_file) as fname:
            obj = pcsv.CsvFile(fname=fname, frow=5)
        assert obj.data() == [[2, 5, 40], [3, 5, 50]]
        with pmisc.TmpFile(write_no_header_file) as fname:
            obj = pcsv.CsvFile(fname=fname, has_header=False, frow=2)
    ...
    def test_add_dfilter(self):
        ...
        with pmisc.TmpFile(write_file) as fname:
            ...
            assert obj.data(filtered=True) == [[4]]
            # Two single elements
            obj = pcsv.CsvFile(fname=fname, dfilter={'Result':20})
            assert obj.data(filtered=True) == [[1, 4, 20]]
    ...
    def test_cols(self):
        ...
        with pmisc.TmpFile(write_file) as fname:
            obj = pcsv.CsvFile(fname=fname, dfilter=[0, 2])
        assert obj.cols() == 3
    ...
    def test_data_exceptions(self):
        ...
        with pmisc.TmpFile(write_empty_cols) as fname:
            obj = pcsv.CsvFile(fname=fname, has_header=False, frow=1)
        assert obj.data() == [
    ...
    def test_dsort(self):
        ...
        with pmisc.TmpFile(write_sort_file) as fname:
            obj = pcsv.CsvFile(fname=fname)
    ...
    def test_replace(self):
        ...
        with pmisc.TmpFile(write_file) as fname:
            obj = pcsv.CsvFile(fname=fname, dfilter=({'Ctrl':2}, [0, 2]))
    ...
    def test_write(self):
        ...
        with pmisc.TmpFile(write_file) as fname:
            obj = pcsv.CsvFile(fname=fname, dfilter={'Ctrl':1})
        ofname = fname
        with pmisc.TmpFile() as fname:
            obj.cfilter = 'Result'
    ...
    def test_write_exceptions(self):
        ...
        obj.reset_dfilter()
        with mock.patch('pmisc.make_dir', side_effect=mock_make_dir):
    ...
    def test_rfilter(self):
        ...
        with pmisc.TmpFile(write_file) as fname:
            ...
            obj = pcsv.CsvFile(fname=fname, dfilter={'Ctrl':2, 'Ref':5})
        ...
        with pmisc.TmpFile(write_str_cols_file) as fname:
            obj = pcsv.CsvFile(fname=fname, dfilter={'Ctrl':'low'})
